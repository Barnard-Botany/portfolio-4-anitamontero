...
- /bioclim gives codes for biological climate data 
...
This is the Iberian peninsula, where Spain and Portugal are located.

2. Is altitude a climate variable? How do the peninsula maps plotted above support your answer?
Yes altitude is a climate variable because it affects the climate (temperature, rainfall, wind patterns etc.) that organisms at any given altitude experience. It is clear from the above example that high altitudes (mountain ranges) and low temperatures consistently co-occur. From this we can hypothesize that altitude affects temperature (and likely many other climate variables) in the Iberian peninsula. 
...
J_pauciflora <- BIEN_occurrence_species(species = "Jungia pauciflora")
J_polita <- BIEN_occurrence_species(species = "Jungia polita")
points(cbind(J_pauciflora$longitude, J_pauciflora$latitude), col="purple", pch=20, cex=.8)
points(cbind(J_polita$longitude, J_polita$latitude), col="red", pch=20, cex=.8)
...
The output tells us that dates in the GBIF database, particularly for entries earlier than 1900, may not be exact. 
...
#Only want rows with complete geocoding data, accomplished with complete.cases argument
dim(Spergularia.small)
...
#We can ask for that data using this format
...
dim(DawnRedwood.df)
DawnRedwood.df
#You should see that five of our 50 lines were left out
```
...
They are accepted names.
```{r}
library("rgbif")
library("spocc")
library("taxize")
results=gnr_resolve(names="Acer saccharum")
head(results$matched_name)
...
Yes they are both angiosperms. They are both of the class Magnoliopsida.

```{r}
classification(c("Acer saccharum", "Malus pumila"), db="itis")
```

3. Angiosperm or otherwise, what plant family do they belong to?
Acer saccharum belongs to the Sapindaceae family and Malus pumila belongs to the Rosaceae family.

4. For at least one genus, look up all the species in that genus.
```{r}
children("Acer", db="col")
```

5. For one species, look up to see if there are subspecies or other infraspecific forms.
```{r}
children("Acer saccharum", db="col")
```
...
```{r}
A_saccharum <- occ(q="Acer saccharum", from="gbif")
A_saccharum
A_saccharum.dat <- (A_saccharum$gbif$data)
head(A_saccharum.dat)
```

```{r}
M_pumila <- occ(q="Malus pumila", from="gbif")
M_pumila
M_pumila.dat <- (M_pumila$gbif$data)
head(M_pumila.dat)
```

7. Clean-up the data-frame:  remove occurrences with missing data and duplicates
```{r}
A_saccharum.df <- occ2df(A_saccharum)
A_saccharum.df
dim(A_saccharum.df)
A_saccharum.df <- A_saccharum.df[complete.cases(A_saccharum.df),]
dim(A_saccharum.df)
A_saccharum.df
A_saccharum.udf <- unique(A_saccharum.df)
dim(A_saccharum.udf)
```

```{r}
M_pumila.df <- occ2df(M_pumila)
M_pumila.df
dim(M_pumila.df)
M_pumila.df <- M_pumila.df[complete.cases(M_pumila.df),]
dim(M_pumila.df)
M_pumila.df
M_pumila.udf <- unique(M_pumila.df)
dim(M_pumila.udf)
```
...
```{r}
all.worldclim <- raster::getData("worldclim", res = 10, var = "bio")
peninsula.worldclim <- crop(all.worldclim, extent(-120, -50, 20 , 50))
plot(peninsula.worldclim[["bio1"]])
points(cbind(M_pumila_sub$longitude, M_pumila_sub$latitude), col="purple", pch=20, cex=.8)
```
...
I do not think that the species is endemic to North America. There are occurance records in Europe, Australia, and New Zeland. However it is possiable that the species is endemic to North America and was spread to other parts of the world through colonialism. 

```{r}
library(dplyr)
M_pumila_sub <- filter(M_pumila.df, name == "Malus pumila Mill.")
```

```{r}
all.worldclim <- raster::getData("worldclim", res = 10, var = "bio")
plot(all.worldclim[["bio1"]])
points(cbind(M_pumila_sub$longitude, M_pumila_sub$latitude), pch = 16, cex = 0.5, col = "red")
```
...
```{r}
A_buergerianum	 <- BIEN_occurrence_species(species = "Acer buergerianum")
all.worldclim <- raster::getData("worldclim", res = 10, var = "bio")
plot(all.worldclim[["bio1"]])
points(cbind(A_buergerianum$longitude, A_buergerianum$latitude), pch = 16, cex = 0.5, col = "red")
```

###11. Then re-map on an appropriately cropped map.
```{r}
all.worldclim <- raster::getData("worldclim", res = 10, var = "bio")
peninsula.worldclim <- crop(all.worldclim, extent(-130, -70, 30 , 50))
plot(peninsula.worldclim[["bio1"]])
points(cbind(A_buergerianum$longitude, A_buergerianum$latitude), col="purple", pch=20, cex=.8)
```

ONE LAST THING
12. Don't forget to identify the peninsula that we are working with!
The peninsula in the earlier examples was the Iberian peninsula. For my Acer plotting I used North America. 
